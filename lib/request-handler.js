  ...
  new Appointment()
    ...
    .query((qb) => {
      qb.whereBetween('start_date_time', [summaryStartDateTime, summaryEndDateTime])
        .orWhereBetween('end_date_time', [summaryStartDateTime, summaryEndDateTime])
    })
    .fetchAll({
    ...
    .then(appointments => {
      ...
      appointments.forEach(appointment => {
        ...
        let startOfDate = moment(summaryStartDateTime);
        let endOfDate = moment(summaryEndDateTime);
        let diff;

        if(startOfDate >= start) {
          diff = moment.duration(end.diff(startOfDate));
        } else if(endOfDate <= end) {
          diff = moment.duration(endOfDate.diff(start));
        } else {
          diff = moment.duration(end.diff(start));
        }

        let diffInMins = Math.round(diff.asMinutes());
