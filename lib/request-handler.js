...
requestHandler.getSummary = function(req, res) {
  // console.log('getDailySummary req params', req.params);
  let summaryStartDateTime = moment(parseInt(req.params.startDateTime, 10)).toString();
  let summaryEndDateTime = moment(parseInt(req.params.endDateTime, 10)).toString();
  // console.log(summaryStartDateTime);
  // console.log('check summaryStartDateTime type', typeof summaryStartDateTime);
  ...
  new Appointment()
    ...
    .then(appointments => {
      // console.log('number of appointments fetched', appointments.length);
      // console.log('getDailySummary appointments', JSON.stringify(appointments));
      let summary = {};
    ...
    new User({email: this.recipient})
      ...
      .then(appointment => {
        ...
        let emailMessage = req.session.user.name + ' has shared the following appointment with you via Planendar!' +
          ...
          '\nClick here to confirm: ' + 'http://localhost:8080/confirmAppointmentPage' + '/:' + req.session.user.id + '/:' + this.sentAppointmentId;
...
requestHandler.confirmAppointmentPage = function (req, res) {
  ...
  this.currentApntId = (req.params.apntid).slice(1,2);
  new Appointment()
